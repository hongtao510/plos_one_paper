    ...
    Init = [0;RE(i); zeros(8,1)]; %Mass_end_1';          %initial condition for numbers in air 
...
%Ing_a_indi(:,i)=repmat((rsh*fsh*TMax/stp),stp,1).*Mass_htf(:,i);      %Ingestion Bathracis
Ing_b_indi(:,i)=repmat((rhm*fhm*TMax/stp),stp,1).*Mass_hand(:,i);     %Ingestion Others

Inhal_indi(:,i)=repmat(((r/Vol)*TMax/stp),stp,1).*Mass_air(:,i);
...
%if i<=4
%Ing_bef(:,i)=cumsum(Ing_a_indi(:,i));
%else
%Ing_bef(:,i)=cumsum(Ing_b_indi(:,i));
%end

%Inhal_bef(:,i)=cumsum(Inhal_indi(:,i));


%Risk_Ing_bef(:,i)=1-exp(-tdr4.*Ing_bef(:,i));
%Risk_Inh_bef(:,i)=1-exp(-tdr3.*Inhal_bef(:,i));         

%Risk_t_bef(:,i)=1-(1-Risk_Inh_bef(:,i)).*(1-Risk_Ing_bef(:,i));
%Risk_tF(:,i)=Risk_t_bef(:,i); %%for figure needs    


if i<=4

Risk_Ing_bef_t(:,i)=exp(-tdr4.*Mass_hand((TMax_1+1):stp,i));
else
Risk_Ing_bef_t(:,i)=exp(-tdr4.*Ing_b_indi((TMax_1+1):stp,i));
end

Risk_Inh_bef_t(:,i)=exp(-tdr3.*Inhal_indi((TMax_1+1):stp,i));  

Safe(:,i)=prod(Risk_Ing_bef_t(:,i).*Risk_Inh_bef_t(:,i));
Risk_tF(:,i)=1-Safe(:,i);
